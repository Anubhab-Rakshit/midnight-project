/**
 * Deploy a Meridian `splitpool` contract instance to a Midnight network.
 *
 * Usage:
 *   npm run deploy:meridian -- --network preprod
 *   npm run deploy:meridian -- --network undeployed
 *
 * The script generates a random invite secret (or uses --invite-secret)
 * and deploys a fresh circle contract. The invite secret is required
 * for members to join via the ZK join circuit.
 */
...
// ─── Parse --invite-secret flag ──────────────────────────────────────────────

function parseInviteSecret(argv: string[]): string | null {
  for (let i = 2; i < argv.length; i++) {
    const arg = argv[i];
    if (arg === '--invite-secret') return argv[i + 1] ?? null;
    if (arg.startsWith('--invite-secret=')) return arg.slice('--invite-secret='.length);
  }
  return null;
}
...
const zkConfigPath = path.resolve(__dirname, '..', 'contracts', 'managed', 'splitpool');
const contractPath = path.join(zkConfigPath, 'contract', 'index.js');

if (!fs.existsSync(contractPath)) {
  console.error('\n❌ Contract not compiled! Run: npm run compile:splitpool\n');
  process.exit(1);
}

const Splitpool = await import(pathToFileURL(contractPath).href);
...
async function createProviders(walletCtx: WalletContext) {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'meridian-circle-state',
      accountId,
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy Meridian circle to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  // Deploy.
  console.log('─── Deploy Meridian Circle ─────────────────────────────────────\n');
  ...
  let compiledContract: any = CompiledContract.make('splitpool', Splitpool.Contract);
  ...
  console.log('  Deploying Meridian circle contract...\n');
  ...
  console.log('  ✅ Meridian circle deployed successfully!\n');
